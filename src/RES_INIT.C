...
int16_t res_changed;
uint8_t *res_lzss_buff;
//==========================================================================
void res_init(uint8_t* buff) {
...
int16_t res_open(const char* filename) {
  ...
  res_decrypt((uint8_t*)&res_header, RES_MAX_ENTRIES * sizeof(RES_HEADER), 128);
  res_changed = 0;
  return 1;
}
//==========================================================================
int16_t res_close(void) {
  ...
  if (!res_active) return RES_NOT_ACTIVE;
  if (res_changed) {
    res_encrypt((uint8_t*)&res_header, RES_MAX_ENTRIES * sizeof(RES_HEADER), 128);
