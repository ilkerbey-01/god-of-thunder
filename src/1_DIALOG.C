...
{ //bridge sign
  ...
  if (!BRIDGE_FIXED)
    actor_speaks(act, 102, 0);
  else
    actor_speaks(act, 110, 0);
  d_restore();
  SEEN_BRIDGE = 1;
  return 0;
}
...
void d_fix_tv(ACTOR *act)
{
  ...
  while (1)
  {
    ...
    if (sel == 1)
    {
      if (!actor_speaks(act, 3013, 0))
        return;
    }
    else if (sel == 2)
    {
      if (!actor_speaks(act, 3014, 0))
        return;
    }
    else
      return;
...
void d_fix_bridge(ACTOR *act)
{
  ...
  {
    if (SEEN_BRIDGE)
      sel = select_option(options_bridge2, d_whichbridge, 0);
    else
    {
    ...
    {
      if (!actor_speaks(act, 3016, 0))
        return;
    }
    else if (sel == 2)
    {
      if (!actor_speaks(act, 3017, 0))
        return;
    }
    else
      break;
